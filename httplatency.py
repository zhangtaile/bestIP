...
def measure_latency(parsed_addr: Tuple[str, int, str], timeout: float = 5.0) -> Tuple[str, float, Optional[str]]:
    ...
    try:
        ...
        return original_address, float('inf'), f"Error: {e}"
        ...
        with open(output_filename, 'w', encoding='utf-8') as f:
            for address, max_latency in sorted_results:
                ...
                if len(parts) >= 2:
                    ip = parts[0]
                    port = parts[1]
                    # Join extra info parts with space
                    info = " ".join(parts[2:])
...
def main():
    ...
    parser.add_argument("-i", "--input", default="http.json", help="Input file containing proxies (JSON or CSV)")
        ...
        if args.input.endswith('.json'):
            with open(args.input, 'r', encoding='utf-8') as f:
                ...
                for item in data:
                    ...
                    try:
                        port_int = int(port)
                        if not validate_ip(ip) or not validate_port(port_int):
                            invalid_count += 1
                            continue
                        
                        # Extract info for display
                        info_parts = [
                            str(item.get('country_cn', '')),
                            str(item.get('city', '')),
                            str(item.get('asOrganization', ''))
                        ]
                        info_parts = [p.strip() for p in info_parts if p and p.strip()]
                        
                        # Reconstruct as a CSV line for backward compatibility with save_results
                        original_line = f"{ip},{port_int},{','.join(info_parts)}"
                        parsed_addresses.append((ip, port_int, original_line))
    ...
    logger.info(f"Loaded {len(parsed_addresses)} proxies from {args.input}.")
